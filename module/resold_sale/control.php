...
class resold_sale extends SecuredControl
{
	/**
	 * 数据库连接。
     * 
    ...
    public function index()
    {
		...
		$where = 'where 1=1 and house_type='.$house_type;
	   	if($_GET) {
			//处理搜索条件
			if($name!=""){
		...
		foreach ($res as $key => $value) {
			$res[$key]=$value;
			$res[$key]['create_date']=date('Y-m-d H:i:s', $value['create_date']);
		}
...
*/
	public function operate()
	{
		if(isset($_POST['ids'])) {
			...
			if($_POST['action']=='del'){
				foreach($_POST['ids'] as $key=>$item){
					$this->pdo->remove('fc_esf', "id=".$item);
				}
	...
	*/
	public function listPic(){
		$esf_id = trim(isset($_GET["resold_sale"])?$_GET["resold_sale"]:0);
		$page_info = "";
...
*/
	public function editPic(){
		...
		if($_SERVER['REQUEST_METHOD']=="POST"){
			$param=$_POST;
			 unset($param['id']);
		$atta = $this->pdo->getRow("select * from fc_esf_attach where id=".$_POST['id']);
		$pic_id = $this->pdo->getRow("select id from fc_esf_pic where attach_id = ".$_POST['id']);
		$param['name'] = isset($_POST['title'])?$_POST['title']:$atta['name'];
		$param['url'] = $atta['url'];
		$param['type']=$atta['type'];
		$param['size'] = $atta['size'];
		$param['checksum']=$atta['checksum'];
		$param['download']=$atta['download'];
		$param['update_at']=time();
		$rlt = $this->pdo->update($param,'fc_esf_attach');
		//$lastId = $this->pdo->getLastInsId();
		$param['title'] = $_POST['title'];
		//$param['attach_id'] = $lastId;
		$param['code'] = $_POST['code'];
		$param['des'] = $_POST['des'];
		$param['flag'] = $_POST['flag'];
		$res = $this->pdo->update($param,'fc_esf_pic',"attach_id=".$_POST['id']);
		if($rlt && $res){
	...
	public function addPic(){
		...
		if($_SERVER['REQUEST_METHOD']=="POST"){
			...
			if(!empty($_FILES['url']['name'])){
				$uptool = new UploadFile($_FILES['url'],"houseImages");
...
public function delpic(){
	...
	if(!empty($rlt)){
		$img_url = $rlt['url'];
		UploadFile::DeleteFile(WEB_ROOT.$img_url);
		...
		*/
		$res_del_attach  = $this->pdo->remove("fc_esf_attach","id=".$attach_id);
		$res_del_pic = $this->pdo->remove("fc_esf_pic","attach_id=".$attach_id);
		if($res_del_attach && $res_del_pic){
			Util::alert_msg("删除图片成功","succeed",$referUrl,1);
		}else{
			Util::alert_msg("删除图片失败!","warning",$_SERVER['HTTP_REFERER'],1);

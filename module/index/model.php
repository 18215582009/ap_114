...
use lib\cache\PhpRedis as PhpRedis;
...
//        exit;
    ...
    public function recom(){
        //资讯信息
        $sql = "select id from web_category where parent_id = 119 and flag = 1";
        $infolei = $this->pdo->getAll($sql);
        $cidKey = array();
        foreach ($infolei as $item){
            $cidKey[] = $item['id'];
        }
        $cidstr=implode(",",$cidKey);

        $index = array();
        $sql = "select ct.id,ct.cid,ct.title,ct.photo,ct.postdate,ct.digest,ctg.name from web_contentindex as ct left join
                web_category as ctg on ct.cid = ctg.id WHERE ct.cid in ($cidstr) ORDER BY ct.digest DESC limit 6";
        $index['info'] = $this->pdo->getAll($sql);

        //政策信息
        $sql = "select ct.id,ct.cid,ct.title,ct.photo,ct.postdate,ct.digest,ctg.name,ct1.intro from web_contentindex as ct left join
                web_category as ctg on ct.cid = ctg.id left join web_content1 as ct1 on ct.id = ct1.id WHERE ct.cid = 162 ORDER BY ct.digest DESC limit 3";
        $index['policy'] = $this->pdo->getAll($sql);

...
//		_log("warn", "jQuery.support.boxModel " + jQuery.support.boxModel);
		...
		// Guess skins path, if not specified
		if(!opts.imagePath) {
			$("script").each( function () {
				var _rexDtLibName = /.*dynatree[^\/]*\.js$/i;
				if( this.src.search(_rexDtLibName) >= 0 ) {
					if( this.src.indexOf("/")>=0 ){ // issue #47
						opts.imagePath = this.src.slice(0, this.src.lastIndexOf("/")) + "/skins/";
					}else{
						opts.imagePath = "skins/";
					}
...
$.ui.dynatree.prototype.options = {
	...
	imagePath: null, // Path to a folder containing icons. Defaults to 'skins/' subdirectory.
